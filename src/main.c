...
void deleteProduct(char code[]) {
    ...
    char line[300]; int lineCount=0; int found=0;

    while (fgets(line, sizeof(line), file)) {
        if (lineCount++ == 0) { // copy header
            fputs(line, temp); 
            continue;
        }
        char existingCode[50]; sscanf(line, "%[^,]", existingCode);
...
int main(){
    ...
    while(1){
        printf("\nType 'exit' to quit, 'list' to view products, or 'delete' to remove a product.\n");

        // ----------------- PRODUCT CODE -----------------
        do{
            printf("Product Code / Command: "); 
        ...
        if(strcmp(productCode,"delete")==0){
            char delCode[50];
            printf("Enter product code to delete: ");
            fgets(delCode,sizeof(delCode),stdin);
            delCode[strcspn(delCode,"\n")]=0;
            deleteProduct(delCode);
            continue;
        }
        ...
        if(productExists(productCode)){
            char choice[10]; 
            printf("Product exists. Update it? (Y/N): ");
